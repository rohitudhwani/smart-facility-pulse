...
A multi-page operational dashboard that demonstrates how a building's BMS (Building Management System) data can be turned into a proactive command center for FM teams. Three modules:
...
[➡️ View the live dashboard](#) <!-- Replace with your Streamlit Cloud URL -->
...
LBNL data is 1-minute resolution (525,600 rows per file × 49 files = 25.7 M rows). Real BMS systems typically log at 5-15 minute intervals. Hourly mean balances signal preservation with practical performance.

### Why filter to occupied hours only
Faults are only observable when the system is actively running (LBNL schedule: Mon-Fri 6am-6pm operate, otherwise setback). Including setback hours dilutes fault signatures with idle data.
...
The current build is a Phase 1 pilot. Planned enhancements documented inside the dashboard methodology pages:
